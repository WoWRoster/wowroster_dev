 ...
 * @version    SVN: $Id: addon_conf.php 2344 2011-09-10 08:35:53Z c.treyce@gmail.com $
...
if( $addon['active'] = '1' )
{
	...
	if( isset($roster->pages[3]) && file_exists($addon['ucp_dir'] . $roster->pages[3] . '.php') )
	{
		$addon['active_file'] = $addon['ucp_dir'] . $roster->pages[3] . '.php';
	}
	else
	{
		$addon['active_file'] = $addon['ucp_dir'] . 'index.php';
	}
	...
	elseif( $addon['config'] != '' )
	{
		if( file_exists($addon['ucp_dir'] . 'config.func.php') )
		{
			include($addon['ucp_dir'] . 'config.func.php');
