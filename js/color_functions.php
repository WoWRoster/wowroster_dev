...
function startMoveSlider(e) {
	...
	if (sliderObjectArray[currentSliderIndex]['onchangeAction']) eval(sliderObjectArray[currentSliderIndex]['onchangeAction']);
}
...
function form_widget_amount_slider(targetElId, formTarget, width, min, max, onchangeAction) {
	...
	sliderObjectArray[slider_counter] = {
		"width": width - slider_handle_image_obj.width,
		"min": min,
		"max": max,
	...
	if (document.body.onmouseup) {
		if (document.body.onmouseup.toString().indexOf('stopMoveSlider') == -1) alert('You already have an onmouseup event assigned to the body tag');
	} else {
	...
	do {
		if (subDiv.tagName == 'DIV' && subDiv.className != 'colorPickerCloseButton') {
			if (subDiv == thisObj) {
				...
				if (activeColorDiv) subDiv.style.zIndex = initZIndex - counter;
				else
...
function createColorPickerTopRow(inputObj) {
	...
	for (var no = 0; no < tabs.length; no++) {
		...
		if (no == 0) tabDiv.style.zIndex = 50;
		else
