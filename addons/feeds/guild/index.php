...
roster_add_css($addon['dir'] . 'style.css','module');
	...
	$member_name = $roster->data['guild_name'];
...
$pageanat = ($addon['config']['page_size'] > '0' ? true : false );
	...
	$queryx = "SELECT * FROM `".$roster->db->table('guild_feed',$addon['basename'])."` WHERE `guild_id`='" . $roster->data['guild_id'] . "' ORDER BY `timestamp` DESC";
	...
	if( $pageanat )
	{
		$q1 = "SELECT * FROM `".$roster->db->table('guild_feed',$addon['basename'])."` WHERE `guild_id`='" . $roster->data['guild_id'] . "'";
