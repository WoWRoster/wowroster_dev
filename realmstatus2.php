 ...
 * @version    SVN: $Id: realmstatus.php 2631 2014-08-21 17:54:35Z ulminia@gmail.com $
...
function img_output( $realmData , $err , $image_path )
{
	...
	$icon_img = ROSTER_BASE . 'img' . DIR_SEP . 'realmstatus' . DIR_SEP . strtolower($realmData['serverpop']) . '-icon.png';
	...
	// If there is no data, then we want to output blank text
	$realmData['servertype'] = $realmData['servertype'] != '' ? $roster->locale->act['rs'][$realmData['servertype']] : $realmData['servertype'];
	$roster_gd->combine_image($pop_img, 38, 18);
	$roster_gd->write_text($roster->config['rs_size_type'], 0, 70, 23, $realmData['servertypecolor'], 0, $typefont, $realmData['servertype'], 'left', array(), $shadow);
	$roster_gd->write_text($roster->config['rs_size_pop'], 0, 297, 47, $realmData['serverpopcolor'], 0, $serverpopfont, $roster->locale->act['rs'][$realmData['serverpop']], 'right', array(), $shadow);
	$roster_gd->write_text($roster->config['rs_size_server'], 0, 72, 48, $roster->config['rs_color_server'], 0, $serverfont, $realmData['server_name'], 'left', array(), $shadow);

	$roster_gd->combine_image($icon_img, 0, 0);
