 ...
 * @version    SVN: $Id: item.php 2620 2013-02-05 14:47:41Z ulminia@gmail.com $
...
class item
{
	...
	function item( $data, $parse_mode=false )
	{
		...
		$this->locale = ( isset($data['locale']) ? $data['locale'] : 'en_US' );
...
// -- makeTooltipHTML() tooltip methods start here //
	...
	{
		$html = '<span style="color:#' . $this->color . ';font-size:15px;font-weight:bold;">' . $this->name . '</span><br />';
		return $html;
	}
	...
	function _getHeroic()
	{
		////$roster->api = new WowAPI('en_US');

		$heroic = $this->attributes['Heroic'];

		if( preg_match( $roster->api->act['tooltip_preg_heroic'], $heroic) )
		{
			$color = '66DD33';
		}
	...
	function _getUpgrade()
	{
		...
		preg_match( $roster->api->act['tooltip_preg_upgrade'], $this->attributes['Upgrade'], $matches);
		//print_r($matches);echo '<br>';
		$this->upgradeMin = $matches[1];
		$this->upgradeMax = $matches[2];
		$upgrade = sprintf($roster->api->act['tooltip_upgrade'],$this->upgradeMin,$this->upgradeMax);
	...
	function _getTrans()
	{
		...
		if( preg_match( $roster->api->act['tooltip_transmogc'], $heroic) )
		{
			$color = 'F58CBA';
		}
	...
	function _getBindType()
	{
		...
		if( preg_match( $roster->api->act['tooltip_preg_soulbound'], $bindtype) )
		{
			$color = '00bbff';
		}
		elseif( preg_match( "/\b" . $roster->api->act['tooltip_accountbound']."/", $bindtype) )
		{
			$color = 'e6cc80';
		}
		else
	...
	{
		//$roster->api = new WowAPI('en_US');
		$html = '<br /><span style="color:#00ff00;">' . sprintf($roster->api->act['tooltip_ienchant'],$this->attributes['Enchantment']) . '</span><br />';
		return $html;
	}
	...
	function _getGemReq()
	{
		foreach( $this->attributes['GemStat'] as $bonus )
		{
			$html .= '<span style="color:#ffd100;">"' . $bonus . '"</span><br />';
		}
		return $html;
	}
	...
	function _getSockets()
	{
		...
		if( isset($this->attributes['Sockets']) )
		{
			...
			{
				$html .= '<img width="10px" height="10px" class="tooltipimage" src="http://wowpvplive.com/Interface/ItemSocketingFrame/ui-emptysocket-'
					   . $roster->api->act['socket_colors_to_en'][strtolower($socket_color)] . '.png" />&nbsp;&nbsp;'
					   . $socket_line . '<br />';
			}
		}
		...
		if( isset($this->attributes['Gems']) )
		{
			...
			foreach( $gems as $gem )
			{
				$html .= '<img width="10px" height="10px" src="http://wowpvplive.com/Interface/Icons/'
					   . $gem['Icon'] . '.png" />'
				...
				if( $this->hasMetaGem && preg_match('/inv_misc_gem_diamond/', $gem['Icon']) )
				{
					...
					{
						if ( preg_match( $roster->api->act['tooltip_preg_meta_requires_min'], $requirement, $matches) )
						{
							$tmp = $roster->api->act['gem_colors_to_en'];
						...
						elseif ( preg_match( $roster->api->act['tooltip_preg_meta_requires_more'], $requirement, $matches) )
						{
							$tmp = $roster->api->act['gem_colors_to_en'];
	...
	function _getDurability()
	{
		...
		$html = '<br />'.$roster->api->act['tooltip_durability'] . '&nbsp;';
	...
	function _getRequiredClasses()
	{
		...
		foreach( $this->attributes['Class'] as $class )
		{
			...
			$html .= '<span style="color:#'. $roster->api->act['class_colorArray'][$class_c] . ';">' . $class_c . '</span>';
			if( $count > $i )
			{
				$html .= ', ';
			}
	...
	function _getRequired()
	{
		...
		{
			if( preg_match($roster->api->act['requires_level'], $val) )
			{
	...
	function _makeTooltipHTML()
	{
		...
		else
		{
			...
			if( isset($this->attributes['GemStat']) )
			{
				$html_tt .= $this->_getGemReq();
			}
	...
	function _parseTooltipFull( $itemid, $enchant=false, $gem1=false, $gem2=false, $gem3=false)
	{
		...
		if( $gem1 || $gem2 || $gem3 )
		{
			...
			foreach( $gems as $gem )
			{
				if( $gem != 0 )
				{
					$g = $this->fetchGem($gem);
					//print_r($g);
					$tt['Attributes']['Gems'][$i] = $g;
					if( isset($tt['Attributes']['Gems'][$i]['Bonus']) )
					{
						$str = ucfirst($g['Color']).' Socket';
						$tooltip = str_replace( $str, $tt['Attributes']['Gems'][$i]['Bonus'], $tooltip);
		...
		if( preg_match($roster->api->act['tooltip_preg_tempenchants'], $tooltip, $matches) )
		{
		...
		if( preg_match($roster->api->act['tooltip_preg_socketbonus'], $tooltip, $matches) )
		{
		...
		if (preg_match($roster->api->act['tooltip_garbage8'], $tooltip, $matches))
		{
			$tooltip = str_replace( $matches[0], '', $tooltip );
			$tooltipWithoutColoredLines = str_replace( $matches[0], '', $tooltipWithoutColoredLines );
		}
...
/*
			...
			if( $enchant < 2000 && preg_match($roster->api->act['tooltip_preg_reinforcedarmor'], $tooltipWithoutColoredLines, $matches) )
			{
				$tooltip = str_replace( $matches[1], '', $tooltip );
				$tt['Attributes']['Enchantment'] = $matches[1];
			}
			elseif( preg_match('/\n(.+?)\n(?:' . $roster->api->act['tooltip_durability'] . '|\s\s' . $roster->api->act['tooltip_reg_requires'] . '|' . substr($roster->api->act['tooltip_preg_emptysocket'], 1, strlen($roster->api->act['tooltip_preg_emptysocket']) - 3 ) . ')/i', $tooltipWithoutColoredLines, $matches) )
			{
				$tooltip = str_replace( $matches[1], '', $tooltip );
				$tt['Attributes']['Enchantment'] = $matches[1];
			}
			elseif( preg_match( '/\n(.+)\n' . $roster->api->act['tooltip_reg_requires'] . '/i', $tooltipWithoutColoredLines, $matches) )
			{
				$tooltip = str_replace( $matches[1], '', $tooltip );
				$tt['Attributes']['Enchantment'] = $matches[1];
			}
			...
			*/
			if( preg_match($roster->api->act['tooltip_preg_enchant'], $tooltipWithoutColoredLines, $matches) )
			{
		...
		$tooltip = str_replace($roster->api->act['tooltip_transmoga'], $roster->api->act['tooltip_transmogb'], $tooltip);
		$setpiece=0;
		...
		foreach( $tooltip as $line )
		{
			...
			{
				if ( !isset($tt['Attributes']['BaseStats'][$matches[2]]) && $matches[2] != $roster->api->act['armor'])
				{
					$tt['Attributes']['BaseStats'][$matches[2]] = $matches[0];
				}
				elseif ($matches[2] == $roster->api->act['armor'] )
				{
					$tt['Attributes']['ArmorClass']['Line'] = $matches[0];
					$tt['Attributes']['ArmorClass']['Rating'] = $matches[1];
				}
			}
			elseif( preg_match( $roster->api->act['tooltip_preg_use'], $line) )
			{
				//Use:
				$tt['Effects']['Use'][] = $line;
			}
			elseif( preg_match('/^([,0-9]+)\s'.$roster->api->act['armor'].'$/', $line, $matches) )
			{
				//echo '<pre>';print_r($matches);echo '</pre>';
				$tt['Attributes']['ArmorClass']['Line'] = $matches[0];
				$tt['Attributes']['ArmorClass']['Rating'] = $matches[1];
			...
			elseif( preg_match( "/" . $roster->api->act['tooltip_reg_requires'] . "/i", $line) )
			{
				//Requires
			...
			elseif( preg_match( "/" . $roster->api->act['tooltip_reforged'] . "/i", $line) )
			{
				//Reforged
				$tt['Attributes']['Requires'][] = $line;
			}
			...
			elseif( preg_match($roster->api->act['tooltip_preg_item_level'], $line, $matches) )
			{
				//Item Level
				$tt['Attributes']['ItemLevel']['Line'] = $matches[0];
				$tt['Attributes']['ItemLevel']['Level'] = $matches[1];
			}
			elseif( preg_match($roster->api->act['tooltip_preg_item_equip'], $line) )
			{
				if( preg_match($roster->api->act['tooltip_preg_chance'], $line) )
				{
					//Chance
			...
			elseif( preg_match( $roster->api->act['tooltip_preg_chance_hit'], $line) )
			{
				$tt['Effects']['ChanceToProc'][] = $line;
			}
			elseif( preg_match( "/\b" . $roster->api->act['tooltip_feral_ap'] . "\b/i", $line) )
			{
				$tt['Effects']['Equip'][] = $line;
			}
			elseif( preg_match( $roster->api->act['tooltip_preg_heroic'], $line) )
			{
				//heroic
				$tt['Attributes']['Heroic'] = $line;
			}
			elseif( preg_match( $roster->api->act['tooltip_preg_upgrade'], $line) )
			{
				//upgrade
				$tt['Attributes']['Upgrade'] = $line;
			}
			elseif( preg_match( $roster->api->act['tooltip_preg_lfr'], $line) )
			{
				//heroic
				$tt['Attributes']['Heroic'] = $line;
			}
			elseif( preg_match( $roster->api->act['tooltip_preg_item_season'], $line) )
			{
				//heroic
				$tt['Attributes']['Heroic'] = $line;
			}
			elseif( preg_match( $roster->api->act['tooltip_transmogc'], $line, $matches) )
			{
				$tt['Attributes']['Trans'] = $matches[0];
			}
			elseif( preg_match( "/\b" . $roster->api->act['tooltip_bind_types'] . "\b/i", $line) )
			{
				//soulbound, bop, quest item etc
				$tt['Attributes']['BindType'] = $line;
			}
			elseif( preg_match($roster->api->act['tooltip_preg_durability'], $line, $matches) )
			{
				$tt['Attributes']['Durability']['Line']= $matches[0];
				$tt['Attributes']['Durability']['Current'] = $matches[2];
				$tt['Attributes']['Durability']['Max'] = $matches[3];
			}
			elseif( preg_match($roster->api->act['tooltip_preg_classes'], $line, $matches) )
			{
				$tt['Attributes']['Class'] = explode(', ', $matches[2]);
				$tt['Attributes']['ClassText'] = $matches[1];
			}
			elseif( preg_match($roster->api->act['tooltip_preg_races'], $line, $matches) )
			{
				$tt['Attributes']['Race'] = explode(', ', $matches[2]);
				$tt['Attributes']['RaceText'] = $matches[1];
			}
			elseif( preg_match($roster->api->act['tooltip_preg_emptysocket'], $line, $matches) )
			{
				// match[1] = color, match[0] full line  Red Socket
				$tt['Attributes']['Sockets'][$matches[1]] = $matches[0];
				$this->isSocketable = true;
			}
			elseif( preg_match($roster->api->act['tooltip_preg_item_set_n'], $line, $matches) )
			{
			...
			elseif( preg_match("/" . $roster->api->act['tooltip_sha'] . "/",$line) )
			{
				$tt['Attributes']['ItemNote'] = $line;
			}
			elseif( preg_match( "/\b" . $roster->api->act['tooltip_unique'] . "\b/i", $line ) )
			{
			...
			{
				$line = explode("\t",$line);
				if(preg_match("/" . $roster->api->act['tooltip_armor_types'] . "/", $line[1] ) )
				{
					$tt['Attributes']['ArmorType'] = $line[1];
					$tt['Attributes']['ArmorSlot'] = $line[0];
					$this->isArmor = true;
				}
				elseif( preg_match($roster->api->act['tooltip_preg_weapon_types'], $line[1] ) )
				{
					$tt['Attributes']['WeaponType'] = $line[1];
					$tt['Attributes']['WeaponSlot'] = $line[0];
					$this->isWeapon = true;
				}
				elseif( preg_match($roster->api->act['tooltip_preg_speed'], $line[1]) )
				{
			...
			elseif( $this->isWeapon && preg_match("/" . $roster->api->act['tooltip_reg_weaponorbulletdps'] . "/", $line) )
			{
				$tt['Attributes']['WeaponDPS'] = $line;
				$this->isWeapon = true;
			}
			elseif( preg_match( $roster->api->act['tooltip_preg_madeby'], $line, $matches ) )
			{
			...
			elseif( preg_match( "/\b" . $roster->api->act['tooltip_source'] . "\b/i", $line ) )
			{
				$tt['Attributes']['Source'] = $line;
			}
			elseif( preg_match( "/\b" . $roster->api->act['tooltip_boss'] . "\b/i", $line ) )
			{
				$tt['Attributes']['Boss'] = $line;
			}
			elseif( preg_match( "/\b" . $roster->api->act['tooltip_droprate'] . "\b/i", $line ) )
			{
			...
			elseif( preg_match($roster->api->act['tooltip_preg_meta_requires'], $line ) )
			{
			...
			else
			{
				...
				if( $line !== '' && $line !== ' '
					&& !preg_match($roster->api->act['tooltip_garbage1'], $line)
					&& !preg_match($roster->api->act['tooltip_garbage2'], $line)
					&& !preg_match($roster->api->act['tooltip_garbage3'], $line)
					&& !preg_match($roster->api->act['tooltip_garbage4'], $line)
					&& !preg_match($roster->api->act['tooltip_garbage5'], $line)
					&& !preg_match($roster->api->act['tooltip_garbage6'], $line)
					&& !preg_match($roster->api->act['tooltip_garbage7'], $line)
					&& !preg_match($roster->api->act['tooltip_preg_dps'], $line)
				)
				{
					// check to match more simpler items
					// also could have fell through the line split in pass1
					if( preg_match( $roster->api->act['tooltip_preg_weapon_types'], $line ) )
					{
						$tt['Attributes']['WeaponSlot'] = $line;
						$this->isWeapon = true;
					}
					elseif( preg_match( "/\b" . $roster->api->act['tooltip_misc_types'] . "\b/i", $line) )
					{
					...
					//
					//check if its a bag
					elseif( preg_match( $roster->api->act['tooltip_preg_bags'], $line, $matches ) )
					{
					...
					elseif( preg_match( $roster->api->act['tooltip_preg_block'], $line, $matches ) )
					{
					...
					elseif( preg_match($roster->api->act['tooltip_preg_armor'], $line, $matches) )
					{
						//echo '<pre>';print_r($matches);echo '</pre>';
						$tt['Attributes']['ArmorClass']['Line'] = $matches[0];
						$tt['Attributes']['ArmorClass']['Rating'] = $matches[1];
					...
					elseif( preg_match( $roster->api->act['tooltip_preg_charges'], $line ) )
					{
						$tt['Attributes']['Charges'] = $line;
					}
					//
					//check if item is a poison
					elseif( preg_match( '/'.$roster->api->act['tooltip_poisoneffect'] . "/", $line ) )
					{
					...
					//
					//is this needed?
					elseif( preg_match( "/\b" . $roster->api->act['tooltip_armor_types'] . "\b/i", $line) )
					{
						$tt['Attributes']['ArmorSlot'] = $line;
						$this->isArmor = true;
					}
					elseif( preg_match( "/\b" . $roster->api->act['tooltip_reg_onlyworksinside'] . "\b/i", $line) )
					{
						$tt['Attributes']['Restrictions'][] = $line;
					}
					elseif( preg_match( "/\b" . $roster->api->act['tooltip_reg_conjureditems']."/", $line) )
					{
						$tt['Attributes']['Conjured'][] = $line;
					}
					elseif( preg_match( $roster->api->act['gem_preg_multicolor'], $line) )
					{
						$tt['Attributes']['GemStat'][] = $line;
					}
					elseif( preg_match( $roster->api->act['gem_preg_singlecolor'], $line) )
					{
						$tt['Attributes']['GemStat'][] = $line;
					}
					elseif( preg_match( $roster->api->act['gem_preg_meta'], $line) )
					{
						$tt['Attributes']['GemStat'][] = $line;
					}
					elseif( preg_match( $roster->api->act['gem_preg_prismatic'], $line) )
					{
						$tt['Attributes']['GemStat'][] = $line;
					}
	...
	//		return $roster->cache->mget($gem_id . $locale);
		...
		$sql = "SELECT `gem_id` AS GemId, `gem_name` AS Name, `gem_color` AS Color, `gem_tooltip` AS Tooltip, "
			 . "`gem_texture` AS Icon, `gem_bonus` AS Bonus, `locale` FROM `api_gems` WHERE `gem_id` = '" . $gem_id . "' AND `locale` = '" . $locale . "';";
		$result = $roster->db->query($sql);
		$gem = $roster->db->fetch($result, SQL_ASSOC);
		//$resultg = db_query($sql);
		//$gem = $resultg->fetchAssoc();
