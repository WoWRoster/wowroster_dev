 ...
 * @version    SVN: $Id: item.php 2598 2012-08-30 22:17:28Z ulminia@gmail.com $
...
class item
{
	...
	var $html_tooltip;
	var $stack_tooltip;
	var $stack_tooltipWithoutColoredLines;
	...
	{
		//72803:0:3999:3999:0:0:0:2120686208:85:128 128 is reforged
		list($itemid, $enchant, $gem1, $gem2, $gem3, $gem4, $_, $_, $level, $reforge) = explode(':', $this->item_id);

		if( $this->isParseMode == 'full' && !strstr($this->name, ':') || $enchant || $gem1 || $gem2 || $gem3 && !$this->isParseMode == 'simple'  )
		{
	...
	function _parseTooltipFull( $itemid, $enchant=false, $gem1=false, $gem2=false, $gem3=false)
	{
		...
		$locale = $this->locale;
		$this->stack_tooltip = $this->tooltip;

		$tt = array();
		$setpiece = null; //flag for parsing

		//cleanup tooltip for parsing
		$this->stack_tooltip = str_replace("\n\n", "\n", $this->stack_tooltip);
		$this->stack_tooltip = str_replace('<br>', "\n",$this->stack_tooltip);
		$this->stack_tooltip = str_replace('<br />', "\n",$this->stack_tooltip);

		// Another filter for non breaking spaces
		// this might be removed once all character data is updated
		$this->stack_tooltip = str_replace(chr(194).chr(160), ' ', $this->stack_tooltip);

		// As enchants can't be in colored lines we make a copy of tooltip without colored lines
		$this->stack_tooltipWithoutColoredLines = $this->stack_tooltip;
		$this->stack_tooltip = preg_replace( '/\|c[a-f0-9]{6,8}(.+?)\|r/', '$1', $this->stack_tooltip );
		$this->stack_tooltipWithoutColoredLines = preg_replace( '/\s\s\|c[a-f0-9]{6,8}.+?\|r\n/', '',$this->stack_tooltipWithoutColoredLines );
		$this->stack_tooltipWithoutColoredLines = preg_replace( '/\|c[a-f0-9]{6,8}.+?\|r\n/', '', $this->stack_tooltipWithoutColoredLines );

		// if any gems get the data for them, remove lines from stack.
		if( $gem1 || $gem2 || $gem3 || $gem4 )
		{
			$gems = array($gem1,$gem2,$gem3,$gem4);
			$i = 1;
			foreach( $gems as $gem )
			{
				if( $gem != 0 )
				{
					...
					{
						$this->stack_tooltip = str_replace( $tt['Attributes']['Gems'][$i]['Bonus'] . "\n", '', $this->stack_tooltip);
						$this->stack_tooltip = str_replace('<span style="color:#ffffff;">' . $tt['Attributes']['Gems'][$i]['Bonus'] . "</span>\n", '', $this->stack_tooltip);
						$this->stack_tooltipWithoutColoredLines = str_replace( $tt['Attributes']['Gems'][$i]['Bonus'] . "\n", '', $this->stack_tooltipWithoutColoredLines);
						$this->stack_tooltipWithoutColoredLines = str_replace('<span style="color:#ffffff;">' . $tt['Attributes']['Gems'][$i]['Bonus'] . "</span>\n", '', $this->stack_tooltipWithoutColoredLines);
					}
					else
					{
		...
		if( preg_match($roster->locale->wordings[$locale]['tooltip_preg_tempenchants'], $this->stack_tooltip, $matches) )
		{
			$this->stack_tooltip = str_replace( $matches[0], '', $this->stack_tooltip );
			$this->stack_tooltipWithoutColoredLines = str_replace( $matches[0], '', $this->stack_tooltipWithoutColoredLines );
		...
		if( preg_match($roster->locale->wordings[$locale]['tooltip_preg_socketbonus'], $this->stack_tooltip, $matches) )
		{
			$this->stack_tooltip = str_replace( $matches[0] . "\n", '', $this->stack_tooltip );
			$this->stack_tooltipWithoutColoredLines = str_replace( $matches[0] . "\n", '', $this->stack_tooltipWithoutColoredLines );
		...
		if( $enchant )
		{
			...
			if( $enchant < 2000 && preg_match($roster->locale->wordings[$locale]['tooltip_preg_reinforcedarmor'], $this->stack_tooltipWithoutColoredLines, $matches) )
			{
				$this->stack_tooltip = str_replace( $matches[1], '', $this->stack_tooltip );
				$tt['Attributes']['Enchantment'] = $matches[1];
			}
			elseif( preg_match('/\n(.+?)\n(?:' . $roster->locale->wordings[$locale]['tooltip_durability'] . '|\s\s' . $roster->locale->wordings[$locale]['tooltip_reg_requires'] . '|' . substr($roster->locale->wordings[$locale]['tooltip_preg_emptysocket'], 1, strlen($roster->locale->wordings[$locale]['tooltip_preg_emptysocket']) - 3 ) . ')/i', $this->stack_tooltipWithoutColoredLines, $matches) )
			{
				$this->stack_tooltip = str_replace( $matches[1], '', $this->stack_tooltip );
				$tt['Attributes']['Enchantment'] = $matches[1];
			}
			elseif( preg_match( '/\n(.+)\n' . $roster->locale->wordings[$locale]['tooltip_reg_requires'] . '/i', $this->stack_tooltipWithoutColoredLines, $matches) )
			{
				$this->stack_tooltip = str_replace( $matches[1], '', $this->stack_tooltip );
				$tt['Attributes']['Enchantment'] = $matches[1];
			}
			...
			//
			elseif( preg_match_all('/\+\d+.+/', $this->stack_tooltipWithoutColoredLines, $matches) )
			{
				//grab the last + stat in the tooltip and call that the enchantment.
				$this->stack_tooltip = str_replace($matches[0][count($matches[0])-1], '', $this->stack_tooltip);
				$tt['Attributes']['Enchantment'] = $matches[0][count($matches[0])-1];
			}
			elseif( preg_match('/([+,0-9]+)\s(.+)/', $this->stack_tooltipWithoutColoredLines, $matches) )
			{
		...
		$this->stack_tooltip = str_replace($roster->locale->wordings[$locale]['tooltip_transmoga'], $roster->locale->wordings[$locale]['tooltip_transmogb'], $this->stack_tooltip);
		$setpiece=0;
		...
		foreach( $tvt as $linet )
		{
			...
			if( preg_match('/^  (.*)$/', $linet) )
			{
				if( strlen($linet) > 4)
				{
					...
					$setpiece++;
					$this->stack_tooltip = str_replace($linet, '', $this->stack_tooltip);
				}
				else
				{
					$setpiece=false;
		...
		$this->stack_tooltip = explode("\n", $this->stack_tooltip);
		
		$tt['General']['Name'] = array_shift($this->stack_tooltip);
		...
		foreach( $this->stack_tooltip as $line )
		{
			//
	...
	/**
	 *
	 * This function removes lined from the stack
	 *	$match is the line to remove
	 *  @return null;
	 */
	function stripStack( $match )
	{
		$this->stack_tooltip = str_replace( $match, '', $this->stack_tooltip );
		$this->stack_tooltipWithoutColoredLines = str_replace( $match, '', $this->stack_tooltipWithoutColoredLines );
	}
	/**
