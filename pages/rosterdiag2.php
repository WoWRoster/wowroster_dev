 ...
 * @version    SVN: $Id: rosterdiag.php 2494 2012-05-27 06:10:03Z c.treyce@gmail.com $
...
include_once(ROSTER_LIB.'rosterdiag.git.php');
	...
	<th class="membersHeader">Misc</th>
			...
			foreach ($files[$directory] as $file => $filedata)
			{
				...
				if (isset($file) && $file != 'newer' && $file != 'severity' && $file != 'tooltip' && $file != 'rollup' && $file != 'rev' && $file != 'date' && $file != 'author' && $file != 'md5' && $file != 'update' && $file != 'diff' && $file != 'missing')
				{
					...
					if (isset($filedata['local']['versionMD5']))
					{
						echo $filedata['local']['versionMD5'];
					}
					else
					{
						echo 'Unknown Sha';
